...
class ElasticUnifiedLogTest {
    ...
    val openedElasticUnifiedLogs = mutable.Map[File, ElasticUnifiedLog]()
    ...
    def testHighWatermarkMetadataUpdatedAfterSegmentRoll(): Unit = {
        ...
        val log = getOrCreateLog(logDir, logConfig)
    ...
    /**
     * This method contained tests with log truncation. However, truncation is not supported in ElasticLog.
     * Therefore, related codes are removed.
     */
    @Test
    def testHighWatermarkMaintenance(): Unit = {
        ...
        assertHighWatermark(6L)
        assertEquals(9L, log.logEndOffset)
        assertEquals(0L, log.logStartOffset)
    }

    private def assertNonEmptyFetch(log: ElasticUnifiedLog, offset: Long, isolation: FetchIsolation): Unit = {
    ...
    private def assertEmptyFetch(log: ElasticUnifiedLog, offset: Long, isolation: FetchIsolation): Unit = {
        val readInfo = log.read(startOffset = offset,
            maxLength = Int.MaxValue,
            isolation = isolation,
            minOneMessage = true)
    ...
    def testTimeBasedLogRollDuringAppend(): Unit = {
        ...
        // create a log
        val log = getOrCreateLog(logDir, logConfig, producerStateManagerConfig = new ProducerStateManagerConfig(24 * 60))
    ...
    def testRollSegmentThatAlreadyExists(): Unit = {
        ...
        log.activeSegment.timeIndex.resize(0)
        ...
        assertTrue(log.activeSegment.timeIndex.maxEntries > 1)
    ...
    def testLogSegmentsCallCorrect(): Unit = {
        ...
        def getSegmentOffsets(log: ElasticUnifiedLog, from: Long, to: Long) = log.logSegments(from, to).map {
            _.baseOffset
        }
        ...
        assertEquals(List.empty[ElasticLogSegment], getSegmentOffsets(log, 10, 10))
        assertEquals(List.empty[ElasticLogSegment], getSegmentOffsets(log, 15, 15))
    ...
    def testLogReinitializeAfterManualDelete(): Unit = {
        ...
        val log = getOrCreateLog(logDir, logConfig, logStartOffset = 500)
     ...
     */
    @Test
    def testProducerExpireCheckAfterClose(): Unit = {
        val scheduler = new KafkaScheduler(1)
        try {
            scheduler.startup()
            val logConfig = LogTestUtils.createLogConfig()
            val log = getOrCreateLog(logDir, logConfig, scheduler = scheduler)
    ...
    /**
     * This test contained active producer check after log truncation. We ignore those related checks.
     */
    @Test
    ...
    /**
     * This method contained activeProducers-check after log truncation. We ignore those related checks.
     */
    @Test
    ...
    /**
     * This method contained latestProducerSnapshotOffset checks after truncation. We ignore those related checks.
     */
    @Test
    ...
    def testRetentionDeletesProducerStateSnapshots(): Unit = {
        ...
        mockTime.sleep(1)
        assertEquals(1, log.listProducerSnapshots().size,
    ...
    def testLoadingLogDeletesProducerStateSnapshotsPastLogEndOffset(): Unit = {
        ...
        val logConfig = LogTestUtils.createLogConfig(segmentBytes = 2048 * 5, retentionBytes = -1, fileDeleteDelayMs = 0)
        val log = getOrCreateLog(logDir, logConfig)
        assertEquals(0, log.listProducerSnapshots().size,
    ...
    /**
     * This test is intended to test producer snapshot after log truncation. We ignore test codes after truncation and
     * leave test method unchanged.
     */
    @Test
    ...
    def testPeriodicProducerIdExpiration(): Unit = {
        ...
        val pid = 23L
        val logConfig = LogTestUtils.createLogConfig(segmentBytes = 2048 * 5)
        val log = getOrCreateLog(logDir, logConfig, producerStateManagerConfig = producerStateManagerConfig,
...
//        createEmptyLogs(logDir, 0)
    ...
    def testAppendAndReadWithNonSequentialOffsets(): Unit = {
        ...
        val log = getOrCreateLog(logDir, logConfig)
        val messageIds = ((0 until 50) ++ (50 until 80)).toArray
    ...
    def testReadWithTooSmallMaxLength(): Unit = {
        ...
        for (i <- 50 until messageIds.max) {
            ...
            // We ignore checks here since maxSize is just a hint.
...
//            assertEquals(1, fetchInfo.records.sizeInBytes)
    ...
    /**
     * This test was modified to test :
     * 1) sync feature of the log appending
     * 2) flushing function of the log
     */
    @Test
     ...
     */
    @Test
    def testThatGarbageCollectingSegmentsDoesntChangeOffset0(): Unit = {
        garbageCollectingSegmentsDoesntChangeOffset0(0)
    }

    @Test
    def testThatGarbageCollectingSegmentsDoesntChangeOffset1(): Unit = {
        garbageCollectingSegmentsDoesntChangeOffset0(1)
    }

    @Test
    def testThatGarbageCollectingSegmentsDoesntChangeOffset25(): Unit = {
        garbageCollectingSegmentsDoesntChangeOffset0(25)
    }

    def garbageCollectingSegmentsDoesntChangeOffset0(messagesToAppend: Int): Unit = {
    ...
    def testMessageSizeCheckInAppendAsFollower(): Unit = {
        ...
        val log = getOrCreateLog(logDir, LogTestUtils.createLogConfig(maxMessageBytes = second.sizeInBytes - 1))
     ...
     * This method is intended to check index after truncation. We ignore those codes and keep the method name.
     */
    @Test
...
//        createEmptyLogs(logDir, 7)
        val log = getOrCreateLog(logDir, LogConfig(), brokerTopicStats = brokerTopicStats)
        moveStartOffset(log, 7)
    ...
    def testAppendToOrReadFromLogInFailedLogDir(): Unit = {
        ...
        append(10)
        // Kind of a hack, but closing the index ensures that the append
        // to the index will fail.
        log.activeSegment.txnIndex.close()
...
//        assertEquals(0, epochCache(log).epochEntries.size, "Epoch entries should have gone.")
    ...
    def shouldDeleteStartOffsetBreachedSegmentsWhenPolicyDoesNotIncludeDelete(): Unit = {
        ...
        val log = getOrCreateLog(logDir, logConfig, brokerTopicStats)
    ...
    private def assertCachedFirstUnstableOffset(log: ElasticUnifiedLog, expectedOffset: Long): Unit = {
    ...
    private def assertValidLogOffsetMetadata(log: ElasticUnifiedLog, offsetMetadata: LogOffsetMetadata): Unit = {
     ...
     * Here, we ignore segments-check after log deletion.
     */
    @Test
    ...
    private def appendAsFollower(log: ElasticUnifiedLog, records: MemoryRecords, leaderEpoch: Int = 0): Unit = {
        records.batches.forEach(_.setPartitionLeaderEpoch(leaderEpoch))
        log.appendAsFollower(records)
    }

    private def getOrCreateLog(dir: File,
        config: LogConfig,
        ...
        keepPartitionMetadataFile: Boolean = true): ElasticUnifiedLog = {
        val log = openedElasticUnifiedLogs.getOrElseUpdate(dir, createLog(dir, config, brokerTopicStats, scheduler, time,
            maxTransactionTimeoutMs, producerStateManagerConfig, producerIdExpirationCheckIntervalMs,
            topicId))
        moveStartOffset(log, logStartOffset)
        log
    }
    ...
    /**
     * Move the start offset of the log to the given offset.
     * @param log The log to move
     * @param newStartOffset The new start offset
     */
    private def moveStartOffset(log: ElasticUnifiedLog, newStartOffset: Long): Unit = {
        if (log.logStartOffset > newStartOffset) {
            throw new IllegalArgumentException(s"Cannot move log's start offset to $newStartOffset since it is larger than the current start offset ${log.logStartOffset}")
        }
        if (log.logStartOffset == newStartOffset) {
            return
        }
        // 1. append util the new start offset
        LogTestUtils.appendNonTransactionalAsLeader(log, (newStartOffset - log.logStartOffset).toInt)
        val oldSegments = log.logSegments.toList
        // 2. roll to the target segment
        log.roll(Some(newStartOffset))
        // 3. delete old segments
        log.removeAndDeleteSegments(oldSegments, false)
        // 4. update HW and log start offset
        log.updateHighWatermark(newStartOffset)
        log.maybeIncrementLogStartOffset(newStartOffset, ClientRecordDeletion)
    }
}

object UnifiedLogTest {
    def allRecords(log: ElasticUnifiedLog): List[Record] = {
    ...
    def verifyRecordsInLog(log: ElasticUnifiedLog, expectedRecords: List[Record]): Unit = {
