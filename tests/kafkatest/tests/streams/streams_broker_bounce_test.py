...
class StreamsBrokerBounceTest(Test):
    ...
    def test_many_brokers_bounce(self, failure_mode, num_failures, metadata_quorum=quorum.remote_kraft):
        """
    ...
    @matrix(failure_mode=["clean_bounce", "hard_bounce"],
            num_failures=[3],
            metadata_quorum=quorum.all_non_upgrade)
    def test_all_brokers_bounce(self, failure_mode, num_failures, metadata_quorum=quorum.remote_kraft):
        """
        Start a smoke test client, then kill a few brokers and ensure data is still received
        Record if records are delivered
        """
