...
class TestVerifiableProducer(Test):
    ...
    @cluster(num_nodes=3)
    @parametrize(producer_version=str(LATEST_0_8_2), metadata_quorum=quorum.remote_kraft)
    @parametrize(producer_version=str(LATEST_0_9), metadata_quorum=quorum.remote_kraft)
    @parametrize(producer_version=str(LATEST_0_10_0), metadata_quorum=quorum.remote_kraft)
    @parametrize(producer_version=str(LATEST_0_10_1), metadata_quorum=quorum.remote_kraft)
    ...
    def test_simple_run(self, producer_version, acks=None, enable_idempotence=False, security_protocol = 'PLAINTEXT',
                        sasl_mechanism='PLAIN', metadata_quorum=quorum.remote_kraft):
        """
