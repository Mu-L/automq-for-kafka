...
class PerformanceServiceTest(Test):
    ...
    @parametrize(version=str(LATEST_0_8_2), new_consumer=False, metadata_quorum=quorum.remote_kraft)
    @parametrize(version=str(LATEST_0_9), new_consumer=False, metadata_quorum=quorum.remote_kraft)
    @parametrize(version=str(LATEST_0_9), metadata_quorum=quorum.remote_kraft)
    @parametrize(version=str(LATEST_1_1), new_consumer=False, metadata_quorum=quorum.remote_kraft)
    @cluster(num_nodes=5)
    @matrix(version=[str(DEV_BRANCH)], metadata_quorum=quorum.all)
    def test_version(self, version=str(LATEST_0_9), new_consumer=True, metadata_quorum=quorum.remote_kraft):
        """
